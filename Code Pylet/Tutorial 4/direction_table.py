...
D2R = (math.pi * 2) / 360
DIRECTION_VECTOR_LOOKUP = list(
    [[math.cos(D2R * degrees), math.sin(D2R * degrees)] for degrees in range(360)])
...
while True:
    ...
    for i in range(360):
        if i == v:
            ...
            pygame.draw.line(DS, (255, 0, 255),
                             (DW_HALF, DH_HALF), (px, py), 1)
            ...
            pygame.draw.line(
                ...
                (255, 255, 255),
                (DW_HALF, DH_HALF),
                (DW_HALF + DIRECTION_VECTOR_LOOKUP[i][0] * 300,
                 DH_HALF + DIRECTION_VECTOR_LOOKUP[i][1] * 300),
                1)

    text = "deg: {0}, dx: {1}, dy: {2}" . format(
        v, DIRECTION_VECTOR_LOOKUP[v][0], DIRECTION_VECTOR_LOOKUP[v][1])
