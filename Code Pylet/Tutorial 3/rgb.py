...
class slider:
    ...
    def draw(self, display):
        ...
        pygame.draw.rect(display, (128, 128, 128),
                         (self.x, self.y - ext, self.w, self.h + (ext * 2)), 0)
        pygame.draw.rect(display, (255, 255, 255),
                         (self.x, self.y, self.w, self.h), 0)
        pygame.draw.rect(display, (128, 0, 0), (self.x, self.y +
                                                self.value - ext, self.w, self.vector + 1 + (ext * 2)), 0)
        text_image = FONT.render("{0}".format(self.value), 1, (0, 0, 255))
        display.blit(text_image, ((self.x + self.w / 2) -
                                  text_image.get_rect().width / 2, self.y - ext + 1))

        label_image = FONT.render(self.label, 1, self.label_color)
        label_x_pos = (self.x + self.w / 2) - \
            (label_image.get_rect().width / 2)
        if self.label_pos == 1:
...
ORBITALS = list([orbital(
    int(math.floor(CENTER_POSITION_XY[0] + math.cos(
        ORBITAL_SPACING_VECTOR * n) * ORBITAL_DISTANCE_FROM_CENTER)),
    int(math.floor(CENTER_POSITION_XY[1] + math.sin(
        ORBITAL_SPACING_VECTOR * n) * ORBITAL_DISTANCE_FROM_CENTER)),
    ORBITAL_SIZE
...
SLIDERS = list([
    slider(SLIDER_X, SLIDER_Y, SLIDER_WIDTH,
           256, 0, 255, "red", (255, 0, 0), 0),
    slider(SLIDER_X + (SLIDER_WIDTH + SLIDER_SPACING), SLIDER_Y,
           SLIDER_WIDTH, 256, 0, 255, "green", (0, 255, 0), 1),
    slider(SLIDER_X + (SLIDER_WIDTH * 2 + SLIDER_SPACING * 2),
           SLIDER_Y, SLIDER_WIDTH, 256, 0, 255, "blue", (0, 0, 255), 0)
])
...
def draw_mixer(d, x=MIXER_X, y=MIXER_Y, w=MIXER_WIDTH, h=MIXER_HEIGHT):
    ...
    pygame.draw.rect(d, (red, green, blue),
                     (x + w3rd, y + h3rd, w3rd, h3rd), 0)
    pygame.draw.rect(d, (0, green, blue),
                     (x + w3rd * 2, y + h3rd, w3rd, h3rd), 0)
...
while True:
    ...
    pygame.draw.circle(DS, (SLIDERS[0].value, SLIDERS[1].value,
                            SLIDERS[2].value), CENTER_POSITION_XY, CENTER_SIZE, 0)
